				 ...
				 bool *isnull)
{
	 ...
	 * initialize metadata
	 */
	// proxy->t_info = infomask;
	IndexTupleProxySetInfoMask(proxy, infomask);
	return proxy;
}
 ...
 */
Datum
nocache_index_tuple_proxy_getattr(IndexTupleProxy itp,
								  int attnum,
								  TupleDesc tupleDesc)
{
	...
	if (IndexTupleProxyHasNulls(itp))
	{
		/*
		...
		bp = (bits8 *) IndexTupleProxyNullsMask(itp);
	...
	tp = (char *) itp->tuple
		+ IndexInfoFindDataOffsetByKind(
			IndexTupleProxyGetInfoMask(itp), itp->tuple_kind);
	...
	if (!slow)
	{
		 ...
		 */
		if (IndexTupleProxyHasVarwidths(itp))
		{
			int			j;
	...
	else
	{
		...
		for (i = 0;; i++)		/* loop exit is at "break" */
		{
			...
			if (IndexTupleProxyHasNulls(itp) && att_isnull(i, bp))
			{
				usecache = false;
