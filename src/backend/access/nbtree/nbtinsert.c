			 ...
			 IndexUniqueCheck checkUnique, Relation heapRel)
{
	...
	// indnkeyatts = IndexRelationGetNumberOfKeyAttributes(rel) - (ictx.isglobal ? 1 : 0);
	/* TODO: check that it doesn't affect something else */
	indnkeyatts = rel->rd_rel->relnatts - (ictx.isglobal ? 1 : 0);
				 ...
				 uint32 *speculativeToken)
{
	...
	*is_unique = true;
	 ...
	 */
	// indnkeyatts = IndexRelationGetNumberOfKeyAttributes(ictx->rel) - (ictx->isglobal ? 1 : 0);
	indnkeyatts = ictx->rel->rd_rel->relnatts - (ictx->isglobal ? 1 : 0);
